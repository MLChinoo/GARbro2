...
namespace GameRes.Formats.LiveMaker
{
    ...
    internal class GalMetaData : AnimationMetaData
    {
        ...
        public static GalMetaData Deserialize (BinaryReader reader)
        {
    ...
    public class GalFormat : ImageFormat
    {
        ...
        public override ImageMetaData ReadMetaData (IBinaryStream stream)
        {
            ...
            if (version > 102)
            {
                ...
                return new GalMetaData {
                    ...
                    Unknown1 = LittleEndian.ToInt32 (header, 0x24),
        ...
        public override void Write (Stream file, ImageData image)
        {
           GalMetaData galMeta = null;
            if (image is PngImageData pngData && pngData.CustomChunks.TryGetValue ("gALh", out var chunk))
            {
                ...
                {
                    galMeta = GalMetaData.Deserialize (reader);
                }
            }
            if (galMeta == null)
            {
                galMeta = new GalMetaData {
                    ...
                    ReservedBytes = new byte[] { 0x11, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00 },
                    Mask = 0xFFFFFFC0u,
                    Unknown1 = 0x00004600,
                    BlockWidth = 16,
                    BlockHeight = 16,
                ...
                switch (selectedCompression)
                {
                    ...
                    case "JPEG (24/32bpp only)": galMeta.Compression = 2; break;
                }
            }
        ...
        private GalMetaData ReadStoredMetadata (BinaryReader reader)
        {
            var meta = new GalMetaData {
                Version         = reader.ReadInt32(),
                FrameCount      = reader.ReadInt32(),
                BPP             = reader.ReadInt32(),
                Shuffled   = 0 != reader.ReadInt32(),
                Compression     = reader.ReadInt32(),
                Mask      = (uint)reader.ReadInt32(),
                BlockWidth      = reader.ReadInt32(),
                BlockHeight     = reader.ReadInt32(),
                Unknown2  = (byte)reader.ReadInt32(),
                FrameMask = (uint)reader.ReadInt32(),
                Frames = new List<GalFrameInfo>()
            };
                   ...
                   int stride, GalMetaData meta, int bpp)
        {
            ...
            if (meta.ReservedBytes != null && meta.ReservedBytes.Length == 9)
                ...
                writer.Write (new byte[] { 0x11, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00 });
        ...
        private void WriteLayerData (BinaryWriter writer, byte[] pixels, GalMetaData meta, int width, int height, int bpp)
        {
        ...
        private void WriteColorMap (BinaryWriter writer, BitmapPalette palette)
        {
        ...
        private void WriteGrayscalePalette (BinaryWriter writer)
        {
            for (int i = 0; i < 256; i++)
            {
                writer.Write ((byte)i);
    ...
    internal class GalReaderWithMeta : GalReader
    {
        ...
        public new void Unpack()
        {
            ...
            for (int i = 0; i < layer_count; ++i)
            {
                ...
                layerInfo.Left = m_input.ReadInt32();
                layerInfo.Top = m_input.ReadInt32();
                layerInfo.Visibility = (byte)m_input.ReadByte();
                layerInfo.TransColor = m_input.ReadInt32();
                layerInfo.Alpha = m_input.ReadInt32();
                layerInfo.AlphaOn = (byte)m_input.ReadByte();
                ...
                if (m_info.Version >= 107)
                    layerInfo.Lock = (byte)m_input.ReadByte();
